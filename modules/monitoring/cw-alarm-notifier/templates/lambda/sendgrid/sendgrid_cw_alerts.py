...
def get_sendgrid_api_key():
    ...
    parse_result = parse_sendgrid_env_var()
    secrets_key = parse_result[0]
    sendgrid_api_key = parse_result[1]
    ...
    try:
        ...
        print("[INFO] Successfully retrieved SendGrid API key.")
        return api_key.strip()

    except Exception as e:
        print(f"[ERROR] Failed to retrieve SendGrid API key: {e}")
        raise

def send_alarm_email_sendgrid(template_data):
    """Send email using SendGrid with rendered template."""
    try:
        if sendGridApikey and sendGridApikey.startswith(("SG.", "PM.")):
            sendgrid_api_key = sendGridApikey  # call api_key direct from Enviroment variable
            logger.info(f"[INFO] SendGrid API key found directly in env: {sendgrid_api_key[:11]}***")
        else:
            sendgrid_api_key = get_sendgrid_api_key() # call from function def get_sendgrid_api_key():
            logger.info(f"[INFO] SendGrid API key retrieved from Secrets Manager: {sendgrid_api_key[:11]}***")
